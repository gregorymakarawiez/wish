...
def validate(request, pk):
    ...
    fields=['validation_choice',
            'validation_text',
            'task_due_date',
            'standard_update_due_date',
            'implementer',
            'mailing_list']
    ...
    # get instance of the employee that is validating the task
    employee = Employee.objects.filter(user__pk=user.pk)[0]
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # set logged user as validator
            task.validator=employee

            # add automatic fields and save instance to db
            task.validate(cleaned_data['validation_choice'])
    ...
    # render form using given html template
    return render(request, 'validate_task.html', {'form': form})
